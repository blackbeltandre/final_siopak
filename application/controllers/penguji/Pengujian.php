    ...
    class Pengujian extends CI_Controller {
        ...
        function cetak($id = 0){
           ...
           //ini_set('memory_limit', '-1');
            ...
            $data['data'] = $this->model->detail($id);
            // $data['file'] = $this->model->get_file($id);
            $data['utama'] = $this->dupak->list_dupak_type($id,$tahun,"UNSUR UTAMA");
            $data['penunjang'] = $this->dupak->list_dupak_type($id,$tahun,"UNSUR PENUNJANG");
            $data['pejabat'] = get_pejabat();
            $nip = $this->uri->segment(4);
            $tahun = date('Y');
            $layak = $this->input->post('layak');
            $cek_status_pak = $this->db->query("select * from status_layak where nip=$nip and tahun=$tahun")->result_array();
